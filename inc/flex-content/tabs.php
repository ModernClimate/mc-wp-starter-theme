  ...
  <ul class="nav <?php echo ( $tab_pos == 'side' ) ? 'nav-pills nav-stacked' : 'nav-tabs'; ?>" role="tablist">
    <?php foreach ($tabs as $index => $tab): ?>
      <li role="presentation" <?php echo ($index === 0) ? "class=\"active\"" : ""; ?>>
        <a href="<?php echo "#{$tab_id}-{$index}"; ?>" role="tab" data-toggle="tab"><?php echo $tab['tab_button']; ?></a>
      </li>
    <?php endforeach; ?>
  </ul>
</div>
...
<div class="<?php echo ( $tab_pos == 'side' ) ? 'col-sm-9' : 'col-xs-12'; ?>">
  <div class="tab-content">
    <?php foreach ($tabs as $index => $tab): ?>
      <div role="tabpanel" class="tab-pane <?php echo ($index === 0) ? "active" : ""; ?>" id="<?php echo "{$tab_id}-{$index}"; ?>">
